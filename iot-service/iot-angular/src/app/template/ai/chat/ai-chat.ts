    ...
    const input = inputValue || this.userInput.nativeElement.value;
		...
		messages.innerHTML += `
			<div class="d-flex justify-content-end align-items-end mb-3">
				<div class="rounded-4 px-3 py-2 bg-inverse bg-opacity-10 text-body mw-75">${input}</div>
		...
		setTimeout(() => {
			...
			messages.innerHTML += `
				<div class="d-flex justify-content-start align-items-end mb-3">
					...
					<div class="rounded-4 px-3 py-2 bg-inverse bg-opacity-10 text-body mw-75"><div class="typing-animation" style="animation-duration: .5s;">${aiResponse}</div></div>
				</div>
			`;
			messages.scrollTop = messages.scrollHeight;
