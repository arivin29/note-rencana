...
export class ThemePanelComponent implements AfterViewInit {
	...
	ngAfterViewInit() {
		...
		if (localStorage) {
			...
			if (localStorage['appDirection']) {
				this.handleSetDirection(localStorage['appDirection']);
			}
	...
	handleToggleDirection(event: MouseEvent, direction: string) {
		event.preventDefault();
	...
	handleSetDirection(direction: string) {
		this.appSettings.appDirection = direction;
		if (localStorage) {
